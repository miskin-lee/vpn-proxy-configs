function main(config) {
  const groups = Array.isArray(config["proxy-groups"]) ? config["proxy-groups"] : [];
  const selectGroup = groups.find(group => group.type === "select");
  const proxy = selectGroup && selectGroup.name;

  if (!proxy) {
    return config;
  }

  config.rules = [
    "DOMAIN-SUFFIX,gov.cn,DIRECT",
    "DOMAIN-SUFFIX,12306.cn,DIRECT",
    "DOMAIN-SUFFIX,icbc.com.cn,DIRECT",
    "DOMAIN-SUFFIX,ccb.com,DIRECT",
    "DOMAIN-SUFFIX,abchina.com,DIRECT",
    "DOMAIN-SUFFIX,boc.cn,DIRECT",
    "DOMAIN-SUFFIX,bankcomm.com,DIRECT",
    "DOMAIN-SUFFIX,cmbchina.com,DIRECT",
    "DOMAIN-SUFFIX,cmbbank.com,DIRECT",
    "DOMAIN-SUFFIX,psbc.com,DIRECT",
    "DOMAIN-SUFFIX,spdb.com.cn,DIRECT",
    "DOMAIN-SUFFIX,cib.com.cn,DIRECT",
    "DOMAIN-SUFFIX,hxb.com.cn,DIRECT",
    "DOMAIN-SUFFIX,cgbchina.com.cn,DIRECT",
    "DOMAIN-SUFFIX,pingan.com,DIRECT",
    "DOMAIN-SUFFIX,pingan.com.cn,DIRECT",
    "DOMAIN-SUFFIX,alipay.com,DIRECT",
    "DOMAIN-SUFFIX,alipayobjects.com,DIRECT",
    "DOMAIN-SUFFIX,unionpay.com,DIRECT",
    "DOMAIN-SUFFIX,99bill.com,DIRECT",
    "DOMAIN-SUFFIX,lianlianpay.com,DIRECT",
    "DOMAIN-SUFFIX,jdpay.com,DIRECT",
    "DOMAIN-SUFFIX,caifutong.com,DIRECT",
    "DOMAIN-SUFFIX,mbank.hsbc.com.cn,DIRECT",
    "DOMAIN-SUFFIX,tianyancha.com,DIRECT",
    "DOMAIN-SUFFIX,eastmoney.com,DIRECT",
    "DOMAIN-SUFFIX,xueqiu.com,DIRECT",
    "DOMAIN-SUFFIX,futu.io,DIRECT",
    "DOMAIN-SUFFIX,10jqka.com.cn,DIRECT",
    "DOMAIN-SUFFIX,gtja.com,DIRECT",
    "DOMAIN-SUFFIX,cninfo.com.cn,DIRECT",
    "DOMAIN-SUFFIX,taobao.com,DIRECT",
    "DOMAIN-SUFFIX,tmall.com,DIRECT",
    "DOMAIN-SUFFIX,jd.com,DIRECT",
    "DOMAIN-SUFFIX,360buy.com,DIRECT",
    "DOMAIN-SUFFIX,jdcloud.com,DIRECT",
    "DOMAIN-SUFFIX,pinduoduo.com,DIRECT",
    "DOMAIN-SUFFIX,yangkeduo.com,DIRECT",
    "DOMAIN-SUFFIX,suning.com,DIRECT",
    "DOMAIN-SUFFIX,gome.com.cn,DIRECT",
    "DOMAIN-SUFFIX,vip.com,DIRECT",
    "DOMAIN-SUFFIX,mogujie.com,DIRECT",
    "DOMAIN-SUFFIX,meituan.com,DIRECT",
    "DOMAIN-SUFFIX,dianping.com,DIRECT",
    "DOMAIN-SUFFIX,eleme.cn,DIRECT",
    "DOMAIN-SUFFIX,ele.me,DIRECT",
    "DOMAIN-SUFFIX,dewu.com,DIRECT",
    "DOMAIN-SUFFIX,poizon.com,DIRECT",
    "DOMAIN-SUFFIX,qq.com,DIRECT",
    "DOMAIN-SUFFIX,wechat.com,DIRECT",
    "DOMAIN-SUFFIX,qpic.cn,DIRECT",
    "DOMAIN-SUFFIX,qlogo.cn,DIRECT",
    "DOMAIN-SUFFIX,weibo.com,DIRECT",
    "DOMAIN-SUFFIX,sinaimg.cn,DIRECT",
    "DOMAIN-SUFFIX,sina.com.cn,DIRECT",
    "DOMAIN-SUFFIX,sina.cn,DIRECT",
    "DOMAIN-SUFFIX,dingtalk.com,DIRECT",
    "DOMAIN-SUFFIX,feishu.cn,DIRECT",
    "DOMAIN-SUFFIX,larksuite.com,DIRECT",
    "DOMAIN-SUFFIX,zhihu.com,DIRECT",
    "DOMAIN-SUFFIX,zhimg.com,DIRECT",
    "DOMAIN-SUFFIX,tieba.com,DIRECT",
    "DOMAIN-SUFFIX,douban.com,DIRECT",
    "DOMAIN-SUFFIX,kuaishou.com,DIRECT",
    "DOMAIN-SUFFIX,gifshow.com,DIRECT",
    "DOMAIN-SUFFIX,yy.com,DIRECT",
    "DOMAIN-SUFFIX,huya.com,DIRECT",
    "DOMAIN-SUFFIX,douyin.com,DIRECT",
    "DOMAIN-SUFFIX,douyinpic.com,DIRECT",
    "DOMAIN-SUFFIX,douyinstatic.com,DIRECT",
    "DOMAIN-SUFFIX,douyincdn.com,DIRECT",
    "DOMAIN-SUFFIX,iesdouyin.com,DIRECT",
    "DOMAIN-SUFFIX,tiktokv.com,DIRECT",
    "DOMAIN-SUFFIX,amemv.com,DIRECT",
    "DOMAIN-SUFFIX,snssdk.com,DIRECT",
    "DOMAIN-SUFFIX,ixigua.com,DIRECT",
    "DOMAIN-SUFFIX,ixiguavideo.com,DIRECT",
    "DOMAIN-SUFFIX,toutiaoimg.com,DIRECT",
    "DOMAIN-SUFFIX,bytedance.com,DIRECT",
    "DOMAIN-SUFFIX,byteoversea.com,DIRECT",
    "DOMAIN-SUFFIX,byteimg.com,DIRECT",
    "DOMAIN-SUFFIX,bytecdn.cn,DIRECT",
    "DOMAIN-SUFFIX,volccdn.com,DIRECT",
    "DOMAIN-SUFFIX,volces.com,DIRECT",
    "DOMAIN-SUFFIX,pstatp.com,DIRECT",
    "DOMAIN-SUFFIX,zjurl.cn,DIRECT",
    "DOMAIN-SUFFIX,xiaohongshu.com,DIRECT",
    "DOMAIN-SUFFIX,xhscdn.com,DIRECT",
    "DOMAIN-SUFFIX,xhslink.com,DIRECT",
    "DOMAIN-SUFFIX,fe-static.com,DIRECT",
    "DOMAIN-SUFFIX,bilibili.com,DIRECT",
    "DOMAIN-SUFFIX,bilivideo.com,DIRECT",
    "DOMAIN-SUFFIX,bilivideo.cn,DIRECT",
    "DOMAIN-SUFFIX,acgvideo.com,DIRECT",
    "DOMAIN-SUFFIX,hdslb.com,DIRECT",
    "DOMAIN-SUFFIX,biliimg.com,DIRECT",
    "DOMAIN-SUFFIX,biliapi.net,DIRECT",
    "DOMAIN-SUFFIX,biliapi.com,DIRECT",
    "DOMAIN-SUFFIX,b23.tv,DIRECT",
    "DOMAIN-SUFFIX,bigfun.cn,DIRECT",
    "DOMAIN-SUFFIX,bigfunapp.cn,DIRECT",
    "DOMAIN-SUFFIX,iqiyi.com,DIRECT",
    "DOMAIN-SUFFIX,qiyi.com,DIRECT",
    "DOMAIN-SUFFIX,youku.com,DIRECT",
    "DOMAIN-SUFFIX,alicdn.com,DIRECT",
    "DOMAIN-SUFFIX,mgtv.com,DIRECT",
    "DOMAIN-SUFFIX,sohu.com,DIRECT",
    "DOMAIN-SUFFIX,itouchtv.cn,DIRECT",
    "DOMAIN-SUFFIX,pptv.com,DIRECT",
    "DOMAIN-SUFFIX,le.com,DIRECT",
    "DOMAIN-SUFFIX,lizhi.fm,DIRECT",
    "DOMAIN-SUFFIX,ximalaya.com,DIRECT",
    "DOMAIN-SUFFIX,qingting.fm,DIRECT",
    "DOMAIN-SUFFIX,kugou.com,DIRECT",
    "DOMAIN-SUFFIX,kuwo.cn,DIRECT",
    "DOMAIN-SUFFIX,netease.com,DIRECT",
    "DOMAIN-SUFFIX,qqmusic.com,DIRECT",
    "DOMAIN-SUFFIX,baidu.com,DIRECT",
    "DOMAIN-SUFFIX,bdimg.com,DIRECT",
    "DOMAIN-SUFFIX,bdstatic.com,DIRECT",
    "DOMAIN-SUFFIX,sogou.com,DIRECT",
    "DOMAIN-SUFFIX,so.com,DIRECT",
    "DOMAIN-SUFFIX,360.cn,DIRECT",
    "DOMAIN-SUFFIX,haosou.com,DIRECT",
    "DOMAIN-SUFFIX,163.com,DIRECT",
    "DOMAIN-SUFFIX,126.com,DIRECT",
    "DOMAIN-SUFFIX,ifeng.com,DIRECT",
    "DOMAIN-SUFFIX,tencent.com,DIRECT",
    "DOMAIN-SUFFIX,qcloud.com,DIRECT",
    "DOMAIN-SUFFIX,myqcloud.com,DIRECT",
    "DOMAIN-SUFFIX,aliyun.com,DIRECT",
    "DOMAIN-SUFFIX,aliyuncs.com,DIRECT",
    "DOMAIN-SUFFIX,taobaocdn.com,DIRECT",
    "DOMAIN-SUFFIX,alibabacloud.com,DIRECT",
    "DOMAIN-SUFFIX,alibaba-inc.com,DIRECT",
    "DOMAIN-SUFFIX,huaweicloud.com,DIRECT",
    "DOMAIN-SUFFIX,obs.cn-north-4.myhuaweicloud.com,DIRECT",
    "DOMAIN-SUFFIX,baidubce.com,DIRECT",
    "DOMAIN-SUFFIX,bcebos.com,DIRECT",
    "DOMAIN-SUFFIX,ucloud.cn,DIRECT",
    "DOMAIN-SUFFIX,qiniucdn.com,DIRECT",
    "DOMAIN-SUFFIX,qbox.me,DIRECT",
    "DOMAIN-SUFFIX,upyun.com,DIRECT",
    "DOMAIN-SUFFIX,chinanet.com,DIRECT",
    "DOMAIN-SUFFIX,chinatelecom.com.cn,DIRECT",
    "DOMAIN-SUFFIX,unicom.com.cn,DIRECT",
    "DOMAIN-SUFFIX,cmcc.com.cn,DIRECT",
    "DOMAIN-SUFFIX,gitee.com,DIRECT",
    "DOMAIN-SUFFIX,csdn.net,DIRECT",
    "DOMAIN-SUFFIX,cnblogs.com,DIRECT",
    "DOMAIN-SUFFIX,oschina.net,DIRECT",
    "DOMAIN-SUFFIX,segmentfault.com,DIRECT",
    "DOMAIN-SUFFIX,51cto.com,DIRECT",
    "DOMAIN-SUFFIX,juejin.cn,DIRECT",
    "DOMAIN-SUFFIX,amap.com,DIRECT",
    "DOMAIN-SUFFIX,autonavi.com,DIRECT",
    "DOMAIN-SUFFIX,baidumap.com,DIRECT",
    "DOMAIN-SUFFIX,didiglobal.com,DIRECT",
    "DOMAIN-SUFFIX,didichuxing.com,DIRECT",
    "DOMAIN-SUFFIX,xiaojukeji.com,DIRECT",
    "DOMAIN-SUFFIX,ctrip.com,DIRECT",
    "DOMAIN-SUFFIX,trip.com,DIRECT",
    "DOMAIN-SUFFIX,fliggy.com,DIRECT",
    "DOMAIN-SUFFIX,qunar.com,DIRECT",
    "DOMAIN-SUFFIX,mafengwo.cn,DIRECT",
    "DOMAIN-SUFFIX,ly.com,DIRECT",
    "DOMAIN-SUFFIX,sf-express.com,DIRECT",
    "DOMAIN-SUFFIX,shunfeng.com,DIRECT",
    "DOMAIN-SUFFIX,jdl.com,DIRECT",
    "DOMAIN-SUFFIX,yuantong.com,DIRECT",
    "DOMAIN-SUFFIX,sto.cn,DIRECT",
    "DOMAIN-SUFFIX,yto.net.cn,DIRECT",
    "DOMAIN-SUFFIX,zto.com,DIRECT",
    "DOMAIN-SUFFIX,ems.com.cn,DIRECT",
    "DOMAIN-SUFFIX,zhongtong.com,DIRECT",
    "DOMAIN-SUFFIX,kuaidi100.com,DIRECT",
    "DOMAIN-SUFFIX,edu.cn,DIRECT",
    "DOMAIN-SUFFIX,xuetangx.com,DIRECT",
    "DOMAIN-SUFFIX,icourse163.org,DIRECT",
    "DOMAIN-SUFFIX,imooc.com,DIRECT",
    "DOMAIN-SUFFIX,khanacademy.cn,DIRECT",
    "DOMAIN-SUFFIX,yuanfudao.com,DIRECT",
    "DOMAIN-SUFFIX,zuoyebang.com,DIRECT",
    "DOMAIN-SUFFIX,jzxx.com,DIRECT",
    "DOMAIN-SUFFIX,100tal.com,DIRECT",
    "DOMAIN-SUFFIX,gaokao.com,DIRECT",
    "DOMAIN-SUFFIX,eol.cn,DIRECT",
    "DOMAIN-SUFFIX,guahao.com,DIRECT",
    "DOMAIN-SUFFIX,haodf.com,DIRECT",
    "DOMAIN-SUFFIX,chunyuyisheng.com,DIRECT",
    "DOMAIN-SUFFIX,dxy.cn,DIRECT",
    "DOMAIN-SUFFIX,xywy.com,DIRECT",
    "DOMAIN-SUFFIX,39.net,DIRECT",
    "DOMAIN-SUFFIX,120.net,DIRECT",
    "DOMAIN-SUFFIX,jdzx.cn,DIRECT",
    "DOMAIN-SUFFIX,wegame.com.cn,DIRECT",
    "DOMAIN-SUFFIX,wegame.com,DIRECT",
    "DOMAIN-SUFFIX,tiangou.com,DIRECT",
    "DOMAIN-SUFFIX,4399.com,DIRECT",
    "DOMAIN-SUFFIX,37.com,DIRECT",
    "DOMAIN-SUFFIX,yy.com,DIRECT",
    "DOMAIN-SUFFIX,7k7k.com,DIRECT",
    "DOMAIN-SUFFIX,2144.cn,DIRECT",
    "DOMAIN-SUFFIX,duowan.com,DIRECT",
    "DOMAIN-SUFFIX,battlenet.com.cn,DIRECT",
    "DOMAIN-SUFFIX,blizzard.cn,DIRECT",
    "DOMAIN-SUFFIX,wanmei.com,DIRECT",
    "DOMAIN-SUFFIX,snda.com,DIRECT",
    "DOMAIN-SUFFIX,sdo.com,DIRECT",
    "DOMAIN-SUFFIX,changyou.com,DIRECT",
    "DOMAIN-SUFFIX,xd.com,DIRECT",
    "DOMAIN-SUFFIX,papegames.com,DIRECT",
    "DOMAIN-SUFFIX,mihoyo.com,DIRECT",
    "DOMAIN-SUFFIX,hoyoverse.com,DIRECT",
    "DOMAIN-SUFFIX,hypergryph.com,DIRECT",
    "DOMAIN-SUFFIX,arknights.cn,DIRECT",
    "DOMAIN-SUFFIX,bilibili.game,DIRECT",
    "DOMAIN-SUFFIX,seasungames.com,DIRECT",
    "DOMAIN-SUFFIX,4399youpai.com,DIRECT",
    "DOMAIN-SUFFIX,netease.com,DIRECT",
    "DOMAIN-SUFFIX,163.com,DIRECT",
    "DOMAIN-SUFFIX,leiting.com,DIRECT",
    "DOMAIN-SUFFIX,perfectworld.com,DIRECT",
    "DOMAIN-SUFFIX,pwrd.com,DIRECT",
    "DOMAIN-SUFFIX,steamchina.com,DIRECT",
    "DOMAIN-SUFFIX,steam.cn,DIRECT",
    "DOMAIN-SUFFIX,ysepay.com,DIRECT",
    "DOMAIN-SUFFIX,tanwan.com,DIRECT",
    "DOMAIN-SUFFIX,giants.cn,DIRECT",
    "DOMAIN-SUFFIX,shengqu.com,DIRECT",
    "DOMAIN-SUFFIX,jj.cn,DIRECT",
    "DOMAIN-SUFFIX,jjgames.com,DIRECT",
    "DOMAIN-SUFFIX,91.com,DIRECT",
    "DOMAIN-SUFFIX,people.com.cn,DIRECT",
    "DOMAIN-SUFFIX,xinhuanet.com,DIRECT",
    "DOMAIN-SUFFIX,cctv.com,DIRECT",
    "DOMAIN-SUFFIX,cnr.cn,DIRECT",
    "DOMAIN-SUFFIX,chinadaily.com.cn,DIRECT",
    "DOMAIN-SUFFIX,globaltimes.cn,DIRECT",
    "DOMAIN-SUFFIX,thepaper.cn,DIRECT",
    "DOMAIN-SUFFIX,toutiao.com,DIRECT",
    "DOMAIN-SUFFIX,toutiaoimg.com,DIRECT",
    "DOMAIN-SUFFIX,zjurl.cn,DIRECT",
    "DOMAIN-SUFFIX,caixin.com,DIRECT",
    "DOMAIN-SUFFIX,yicai.com,DIRECT",
    "DOMAIN-SUFFIX,cls.cn,DIRECT",
    "DOMAIN-SUFFIX,36kr.com,DIRECT",
    "DOMAIN-SUFFIX,huxiu.com,DIRECT",
    "DOMAIN-SUFFIX,sspai.com,DIRECT",
    "DOMAIN-SUFFIX,geekpark.net,DIRECT",
    "DOMAIN-SUFFIX,leiphone.com,DIRECT",
    "DOMAIN-SUFFIX,cnbeta.com,DIRECT",
    "DOMAIN-SUFFIX,solidot.org,DIRECT",
    "DOMAIN-SUFFIX,mi.com,DIRECT",
    "DOMAIN-SUFFIX,miui.com,DIRECT",
    "DOMAIN-SUFFIX,xiaomi.com,DIRECT",
    "DOMAIN-SUFFIX,huawei.com,DIRECT",
    "DOMAIN-SUFFIX,hicloud.com,DIRECT",
    "DOMAIN-SUFFIX,honor.cn,DIRECT",
    "DOMAIN-SUFFIX,oppo.com,DIRECT",
    "DOMAIN-SUFFIX,vivo.com.cn,DIRECT",
    "DOMAIN-SUFFIX,realme.com,DIRECT",
    "DOMAIN-SUFFIX,oneplus.com,DIRECT",
    "DOMAIN-SUFFIX,meizu.com,DIRECT",
    "DOMAIN-SUFFIX,lenovo.com,DIRECT",
    "DOMAIN-SUFFIX,motorola.com.cn,DIRECT",
    "DOMAIN-SUFFIX,zte.com.cn,DIRECT",
    "DOMAIN-SUFFIX,tcl.com,DIRECT",
    "DOMAIN-SUFFIX,hisense.com,DIRECT",
    "DOMAIN-SUFFIX,haier.com,DIRECT",
    "DOMAIN-SUFFIX,midea.com,DIRECT",
    "DOMAIN-SUFFIX,gree.com,DIRECT",
    "DOMAIN-SUFFIX,smzdm.com,DIRECT",
    "DOMAIN-SUFFIX,ithome.com,DIRECT",
    "DOMAIN-SUFFIX,wps.cn,DIRECT",
    "DOMAIN-SUFFIX,kdocs.cn,DIRECT",
    "DOMAIN-SUFFIX,jinshuju.net,DIRECT",
    "DOMAIN-SUFFIX,shimo.im,DIRECT",
    "DOMAIN-SUFFIX,mubu.com,DIRECT",
    "DOMAIN-SUFFIX,wolai.com,DIRECT",
    "DOMAIN-SUFFIX,yuque.com,DIRECT",
    "DOMAIN-SUFFIX,ones.cn,DIRECT",
    "DOMAIN-SUFFIX,tapd.cn,DIRECT",
    "DOMAIN-SUFFIX,modao.cc,DIRECT",
    "DOMAIN-SUFFIX,axure.com.cn,DIRECT",
    "DOMAIN-SUFFIX,chuangkit.com,DIRECT",
    "DOMAIN-SUFFIX,uisdc.com,DIRECT",
    "DOMAIN-SUFFIX,gaoding.com,DIRECT",
    "DOMAIN-SUFFIX,meitu.com,DIRECT",
    "DOMAIN-SUFFIX,meituxiuxiu.com,DIRECT",
    "DOMAIN-SUFFIX,photoshop.com.cn,DIRECT",
    "DOMAIN-SUFFIX,360.com,DIRECT",
    "DOMAIN-SUFFIX,360safe.com,DIRECT",
    "DOMAIN-SUFFIX,antiy.com,DIRECT",
    "DOMAIN-SUFFIX,rising.com.cn,DIRECT",
    "IP-CIDR,1.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,14.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,27.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,36.0.0.0/6,DIRECT,no-resolve",
    "IP-CIDR,42.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,49.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,58.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,60.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,61.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,100.64.0.0/10,DIRECT,no-resolve",
    "IP-CIDR,101.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,103.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,106.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,110.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,112.0.0.0/6,DIRECT,no-resolve",
    "IP-CIDR,116.0.0.0/6,DIRECT,no-resolve",
    "IP-CIDR,120.0.0.0/6,DIRECT,no-resolve",
    "IP-CIDR,123.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,124.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,125.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,163.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,171.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,175.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,180.0.0.0/6,DIRECT,no-resolve",
    "IP-CIDR,182.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,183.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,202.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,210.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,218.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,220.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,222.0.0.0/7,DIRECT,no-resolve",
    "IP-CIDR,223.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,10.0.0.0/8,DIRECT,no-resolve",
    "IP-CIDR,172.16.0.0/12,DIRECT,no-resolve",
    "IP-CIDR,192.168.0.0/16,DIRECT,no-resolve",
    "IP-CIDR,127.0.0.0/8,DIRECT,no-resolve",
    "MATCH," + proxy
  ];

  return config;
}
